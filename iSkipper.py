...
try:
    ...
    element_present = expected_conditions.presence_of_element_located(
        (By.CLASS_NAME, "selectable-item-list-courses")
    )
...
courselist = driver.find_elements(
    By.XPATH, "//ul[@class='selectable-item-list-courses']/li/a/label"
)
...
try:
    timeout = PAGE_CHANGE_TIMEOUT
    element_present = expected_conditions.presence_of_element_located(
        (By.CLASS_NAME, "course-navigation-tabs-container")
    )
    WebDriverWait(driver, timeout).until(element_present)
    print(driver.title)
except TimeoutException:
...
while time.time() - start_time < CLASS_LENGTH_TIMEOUT:
    ...
    try:
        ...
        element_present = expected_conditions.presence_of_element_located(
            (By.CLASS_NAME, "multiple-choice-buttons")
        )
        WebDriverWait(driver, timeout).until(element_present)
    except TimeoutException:
    ...
    answer_button.click()
    print(
        f"{driver.title}: Selected {button_id} by MODE_SELECTOR {MODE_SELECTOR} at {time.ctime()}"
    )

    try:
        print(f"{driver.title}: Waiting for end of question")
        element_present = expected_conditions.visibility_of_element_located(
            (By.ID, "status-text-container-id")
        )
